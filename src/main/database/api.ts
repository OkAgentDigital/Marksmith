...
ipcMain.handle('getSpaces', async () => {
  ...
  // Migrate any legacy "My Space" workspaces to @inbox
  const mySpaceWorkspace = spaces.find(s => s.name === 'My Space' || s.name === '我的空间')
  if (mySpaceWorkspace) {
    await knex('space').where('id', mySpaceWorkspace.id).update({
      name: '@inbox'
    })
    // Refresh spaces after migration
