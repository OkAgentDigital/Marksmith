...
import { initializeVault, getVaultManager } from './vault'
...
async function initializeVaults() {
  try {
    // Initialize the new vault system
    await initializeVault()
    
    const vaultManager = getVaultManager()
    const masterVaultPath = vaultManager.getMasterVaultPath()
    ...
    const defaultWorkspaces = ['@toybox', '@sandbox', '@family', '@public', '@private']
    
    for (const workspaceName of defaultWorkspaces) {
      const workspacePath = vaultManager.getWorkspacePath(workspaceName)
      if (!existsSync(workspacePath)) {
        await vaultManager.createWorkspace(workspaceName)
      }
    }
      ...
      vaults: [
        {
          ...
          name: 'MD Vault',
          systemFolders: systemFolders.map(f => ({
            name: f.displayName,
            path: f.path,
            icon: f.icon,
            description: f.description
          })),
          workspaces: defaultWorkspaces.map(name => ({
            name,
            path: vaultManager.getWorkspacePath(name)
          }))
        }
      ]
    }
...
// Vault document methods
ipcMain.handle('getDocumentPath', async (_, docId: string, workspaceName: string) => {
  const vaultManager = getVaultManager()
  return vaultManager.getDocumentPath(docId, workspaceName)
})

ipcMain.handle('readDocumentContent', async (_, docId: string, workspaceName: string) => {
  const vaultManager = getVaultManager()
  return vaultManager.readDocumentContent(docId, workspaceName)
})
