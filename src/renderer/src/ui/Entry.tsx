...
const Entry = observer(() => {
  ...
  return (
    <div className={`flex h-screen`}>
      <div
        className={`sidebar flex-shrink-0  ${!store.settings.state.fullAgent ? '' : 'invisible opacity-0 w-0 h-0 absolute left-0 top-0 pointer-events-none'}`}
      >
        <SideBar />
      </div>
      <div
        className={`flex-1 flex flex-col w-0 min-w-0 ${!store.settings.state.fullAgent ? 'relative' : 'invisible opacity-0 w-0 h-0 absolute left-0 top-0 pointer-events-none'}`}
      >
        <Nav />
