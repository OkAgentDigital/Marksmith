...
export class SettingsStore extends StructStore<typeof state> {
  ...
  async init() {
    ...
    console.log('🔍 Settings: Starting initialization process')
    ...
    try {
      ...
      console.log('✅ Settings: Database loaded successfully', Object.keys(settings).length, 'settings found')
      
      // Step 2: Load models
      console.log('🤖 Settings: Loading AI models...')
      await this.getModels()
      console.log('✅ Settings: Models loaded successfully', this.state.models.length, 'models found')
      
      // Step 3: Apply loaded settings
      console.log('🔧 Settings: Applying configuration...')
      this.setState((state) => {
        let changes = 0
        ...
        console.log('📝 Settings: Applied', changes, 'configuration changes')
        return state
      })
      ...
      if (this.state.language === 'zh') {
        console.log('🇨🇳 Settings: Setting language to Chinese')
        i18next.changeLanguage('zh')
      } else {
        console.log('🇬🇧 Settings: Setting language to English')
        i18next.changeLanguage('en')
      }
      console.log('✅ Settings: i18n initialized successfully')
      
      // Step 5: Load Mermaid (async, don't block)
      console.log('🎨 Settings: Loading Mermaid diagram library...')
      import('mermaid').then((res) => {
        mermaid = res.default
        mermaid.initialize({
          theme: this.state.dark ? 'dark' : 'default'
        })
        console.log('✅ Settings: Mermaid loaded and initialized')
      }).catch(error => {
        console.warn('⚠️  Settings: Mermaid failed to load', error.message)
      })
      
      // Step 6: Execute callbacks
      console.log('🔗 Settings: Executing', this.callbacks.length, 'pending callbacks...')
      if (this.callbacks.length) {
        for (const callback of this.callbacks) {
          try {
            callback()
          } catch (error) {
            console.error('❌ Settings: Callback failed', error)
          }
        }
        console.log('✅ Settings: All callbacks executed')
        this.callbacks = []
      }
      ...
      console.log('⚠️  Settings: Continuing with default configuration')
      // Set ready to true even if initialization fails to unblock the app
      this.setState({ ready: true })
    }
  }
